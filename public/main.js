...
function resetFareAndBaggageInputs() {
    ...
    // FIX #1: Removed references to non-existent 'baggageParticular' radio element.
    // The baggage option radio group was removed from the HTML; baggage is now
    // controlled directly via #baggageAmountInput, so no reset is needed here.
...
function liveUpdateDisplay(pnrProcessingAttempted = false) {
    ...
    // FIX #2: The 'baggageOption' radio group no longer exists in the HTML.
    // Baggage is always sourced directly from #baggageAmountInput.
    ...
    flights.forEach((flight, i) => {
        ...
        floatingPanel.innerHTML = `
            <input type="text" placeholder="Baggage..."
                   ...
                   oninput="${updateJs.replace(/this.innerText/g, 'this.value')}">
...
const historyManager = {
    ...
    init: function () {
        ...
        document.getElementById('closeHistoryBtn')?.addEventListener('click', () => {
            ...
            // Reset preview to placeholder state for next open
            const previewContent = document.getElementById('previewContent');
            if (previewContent) {
                previewContent.innerHTML = '<p id="previewPlaceholder" style="text-align:center;margin-top:50px;color:var(--text-secondary);">Select an item from the list to view details.</p>';
            }
            document.querySelectorAll('.history-item').forEach(el => el.classList.remove('selected'));
            // On mobile: re-hide the panel
        ...
        document.getElementById('historyList')?.addEventListener('click', (e) => {
            ...
            // "Use This" button — load PNR into input and close modal
            ...
            // Clicking anywhere else on the row — show preview
            document.querySelectorAll('.history-item').forEach(el => el.classList.remove('selected'));
            ...
            // Find previewContent — create it if missing (handles old HTML deployments)
            let previewContent = document.getElementById('previewContent');
            if (!previewContent) {
                previewContent = document.createElement('div');
                previewContent.id = 'previewContent';
                previewContent.className = 'preview-content';
                document.getElementById('historyPreviewPanel').appendChild(previewContent);
            }

            // Clear placeholder and show entry or a not-found message
            if (!entry) {
                previewContent.innerHTML = '<p style="color:var(--danger-red);padding:20px;">Entry not found in history.</p>';
            } else {
                const screenshotHtml = entry.screenshot
                    ? `<img src="${entry.screenshot}" alt="Itinerary Screenshot" style="max-width:100%;height:auto;display:block;border:1px solid #ddd;">`
                    : `<p style="color:var(--text-secondary);font-style:italic;">No screenshot available for this entry.</p>`;

                previewContent.innerHTML = `
                    <h4 style="margin:0 0 10px 0;">Screenshot</h4>
                    ${screenshotHtml}
                    <hr style="margin:15px 0;">
                    <button class="copy-pnr-btn" style="background:var(--secondary-dark);color:#fff;padding:8px 14px;border:none;border-radius:4px;cursor:pointer;font-weight:600;margin-bottom:10px;">
                        📋 Copy Raw PNR
                    </button>
                    <pre style="white-space:pre-wrap;word-break:break-all;background:var(--bg-main);padding:12px;border-radius:4px;font-size:12px;max-height:200px;overflow-y:auto;">${entry.pnrText || '(no PNR text saved)'}</pre>
                `;

                // Wire up the copy button (inline, no global listener needed)
                const copyBtn = previewContent.querySelector('.copy-pnr-btn');
                if (copyBtn) {
                    copyBtn.addEventListener('click', () => {
                        navigator.clipboard.writeText(entry.pnrText || '').then(() => {
                            copyBtn.textContent = '✅ Copied!';
                            setTimeout(() => copyBtn.textContent = '📋 Copy Raw PNR', 1500);
                        }).catch(() => copyBtn.textContent = 'Copy failed');
                    });
                }
            }

            // On mobile: slide the panel in via the hidden class
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function syncTransitTimeFormats(e) {
        ...
        saveOptions();
        if (lastPnrResult) handleConvertClick();
    ...
    // --- Email display (element may not be in HTML, guarded with ?.) ---
    ...
    classBtns.forEach(btn => {
        ...
        btn.addEventListener('click', async (e) => {
            ...
            if (globalClassOverride === val) {
                ...
                else if (typeof lastPastedPNR !== 'undefined' && lastPastedPNR) pnrDataToSend = lastPastedPNR;
