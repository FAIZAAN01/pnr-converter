...
function displayResults(pnrResult, displayPnrOptions, fareDetails, baggageDetails, pnrProcessingAttempted) {
    ...
    if (flights.length > 0) {
        ...
        flights.forEach((flight, i) => {
            ...
            const departureString = `${flight.departure.airport}${depTerminalDisplay} - ${flight.departure.city} (${flight.departure.countryCode}), ${flight.departure.name} at ${flight.departure.time}`;
            const arrivalString = `${flight.arrival.airport}${arrTerminalDisplay} - ${flight.arrival.city} (${flight.arrival.countryCode}), ${flight.arrival.name} at ${flight.arrival.time}${arrivalDateDisplay}`;
