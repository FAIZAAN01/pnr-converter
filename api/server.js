...
function parseGalileoEnhanced(pnrText, options) {
    ...
    // --- STATEFUL PARSING LOGIC (Unchanged) ---
    ...
    // --- START OF FIX ---
    // The final logic loop where the transit time is calculated.
    if (flights.length > 0) {
        flights[0].direction = 'Outbound';
        for (let i = 1; i < flights.length; i++) {
            const prev = flightMoments[i - 1]; 
            const curr = flightMoments[i];
            if (prev.arrivalMoment.isValid() && curr.departureMoment.isValid()) {
                ...
                // This condition calculates the display string and the raw minutes.
                if (totalMinutes > 0 && totalMinutes < (24 * 60)) {
                    flights[i].transitTime = `${String(Math.floor(duration.asHours())).padStart(2, '0')}h ${String(duration.minutes()).padStart(2, '0')}m`;
                    
                    // **THIS IS THE FIX:** We must also save the raw minute value.
